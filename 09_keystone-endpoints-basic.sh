...
#  Name:        09_keystone-endpoints-basic.sh
#  Author:      Artem Andreev
#  Email:       aandreev@mirantis.com
#  Edited by:   akirilochkin@mirantis.com
#  Date:        22.05.2013
#  Version:     0.2
#  Description: Keystone endpoints create script "09_keystone-endpoints-basic.sh" from "Openstack grizzly install package"
#
...
# Host address
HOST_IP=${keystone_host:-localhost}
EXT_HOST_IP=${keystone_pub_host:-localhost}
...
for i in compute volume image object-store identity ec2 network; do
  id=`mysql -u "${mysql_user}" -p"${mysql_password}" "${keystone_db}" -ss -e "SELECT id FROM service WHERE type='"$i"';"` || exit 1
